 ...
 * cuckoo hash map using 3 hashes, random walking,
 * and a small stash for problematic keys. No allocation is done except when growing
 * the table size. <br>
 * <br>
 ...
 * the chances the map will have to rehash to the
...
public class IntIntMap implements Iterable<IntIntMap.Entry> {
    ...
    private void push(int insertKey, int insertValue, int index1, int key1, int index2,
                      int key2, int index3, int key3) {
        int[] keyTable = this.keyTable;
    ...
    /**
     * Returns the key's current value and increments the stored value. If the key is
     * not in the map, defaultValue + increment is
     * put into the map.
     */
     ...
     * the same iterator instance is returned each time
     * this method is called. Use the {@link Entries} constructor for nested or
