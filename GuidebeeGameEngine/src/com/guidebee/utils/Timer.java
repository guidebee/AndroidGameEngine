...
public class Timer {
		...
		synchronized (tasks) {
			for (int i = 0, n = tasks.size; i < n; i++) {
				...
				if (task.repeatCount != CANCELLED) {
					...
					GameEngine.app.postRunnable(task);
				}
				if (task.repeatCount == CANCELLED) {
					tasks.removeIndex(i);
					i--;
	...
	/** Manages the single timer thread. Stops thread on libgameengine application pause and dispose, starts thread on resume.
	 * @author Nathan Sweet */
	static class TimerThread implements Runnable, LifecycleListener {
		...
		public TimerThread () {
			GameEngine.app.addLifecycleListener(this);
			resume();
		}
				...
				synchronized (instances) {
					...
					for (int i = 0, n = instances.size; i < n; i++) {
						try {
							...
							throw new GameEngineRuntimeException("Task failed: " + instances.get(i).getClass().getName(), ex);
						}
					}
		...
		public void dispose () {
			pause();
			GameEngine.app.removeLifecycleListener(this);
			thread = null;
			instances.clear();
			instance = null;
		}
	...
	private static void checkStatics () {
		// fix for https://github.com/libgameengine/libgameengine/issues/1548, in case the statics are left overs...
		synchronized (instances) {
			if (thread != null) {
				if (thread.app != GameEngine.app) {
					thread = null;
					instances.clear();
					instance = null;
				}
