...
public class Bezier<T extends Vector<T>> implements Path<T> {
     ...
     */
    public static <T extends Vector<T>> T linear(final T out, final float t,
                                                 final T p0, final T p1, final T tmp) {
        // B1(t) = p0 + (p1-p0)*t
     ...
     * @return The value specified by out for chaining
     */
    public static <T extends Vector<T>> T linear_derivative(final T out,
                                                            final float t, final T p0,
                                                            final T p1, final T tmp) {
        // B1'(t) = p1-p0
     ...
     * @return The value specified by out for chaining
     */
    public static <T extends Vector<T>> T quadratic(final T out, final float t,
                                                    final T p0, final T p1,
                                                    final T p2, final T tmp) {
        // B2(t) = (1 - t) * (1 - t) * p0 + 2 * (1-t) * t * p1 + t*t*p2
        final float dt = 1f - t;
        return out.set(p0).scl(dt * dt).add(tmp.set(p1).scl(2 * dt * t))
     ...
     * @return The value specified by out for chaining
     */
    public static <T extends Vector<T>> T quadratic_derivative(final T out,
                                                               final float t, final T p0,
                                                               final T p1, final T p2,
                                                               final T tmp) {
     ...
     * @return The value specified by out for chaining
     */
    public static <T extends Vector<T>> T cubic(final T out, final float t,
                                                final T p0, final T p1,
                                                final T p2, final T p3,
                                                final T tmp) {
        // B3(t) = (1-t) * (1-t) * (1-t) * p0 + 3 * (1-t) * (1-t) *
        // t * p1 + 3 * (1-t) * t * t * p2 + t * t * t * p3
        final float dt = 1f - t;
        final float dt2 = dt * dt;
        final float t2 = t * t;
        return out.set(p0).scl(dt2 * dt).add(tmp.set(p1).scl(3 * dt2 * t))
     ...
     * @return The value specified by out for chaining
     */
    public static <T extends Vector<T>> T cubic_derivative(final T out,
                                                           final float t,
                                                           final T p0, final T p1,
                                                           final T p2,
                                                           final T p3, final T tmp) {
        // B3'(t) = 3 * (1-t) * (1-t) * (p1 - p0) + 6 * (1 - t) * t *
        // (p2 - p1) + 3 * t * t * (p3 - p2)
        final float dt = 1f - t;
        final float dt2 = dt * dt;
        final float t2 = t * t;
        return out.set(p1).sub(p0).scl(dt2 * 3).add(tmp.set(p2).sub(p1)
                .scl(dt * t * 6)).add(tmp.set(p3).sub(p2).scl(t2 * 3));
    }
    ...
    public Bezier set(final T[] points, final int offset, final int length) {
        if (length < 2 || length > 4)
            throw new GameEngineRuntimeException("Only first, second and " +
                    "third degree Bezier curves are supported.");
    ...
    public Bezier set(final Array<T> points, final int offset, final int length) {
        if (length < 2 || length > 4)
            throw new GameEngineRuntimeException("Only first, second and third" +
                    " degree Bezier curves are supported.");
    ...
    public T valueAt(final T out, final float t) {
        ...
        else if (n == 4) cubic(out, t, points.get(0), points.get(1),
                points.get(2), points.get(3), tmp);
        return out;
    }
    ...
    public T derivativeAt(final T out, final float t) {
        ...
        if (n == 2)
            ...
            quadratic_derivative(out, t, points.get(0), points.get(1),
                    points.get(2), tmp);
        else if (n == 4) cubic_derivative(out, t, points.get(0), points.get(1),
                points.get(2), points.get(3), tmp);
        return out;
    }
