 ...
 * {@link VertexAttributes} instance. The vertices are held either in
 * VRAM in form of vertex buffer objects or in RAM in form of vertex arrays.
 * The former variant is more performant and is preferred
 * over vertex arrays if hardware supports it.
 * </p>
 * <p>
 * <p>
 * Meshes are automatically managed. If the OpenGL context is lost all vertex
 * buffer objects get invalidated and must be reloaded
 * when the context is recreated. This only happens on Android when a user
 * switches to another application or receives an incoming
 * call. A managed Mesh will be reloaded automagically so you don't have to
 * do this manually.
 * </p>
 * <p>
 * <p>
 * A Mesh consists of vertices and optionally indices which specify which
 * vertices define a triangle. Each vertex is composed of
 * attributes such as position, normal, color or texture coordinate. Note
 * that not all of this attributes must be given, except
 * for position which is non-optional. Each attribute has an alias which
 * is used when rendering a Mesh in OpenGL ES 2.0. The alias
 * is used to bind a specific vertex attribute to a shader attribute.
 * The shader source and the alias of the attribute must match
...
public class Mesh implements Disposable {
     ...
     * list of all meshes *
     */
    static final Map<Application, Array<Mesh>>
     ...
     *
     * @param isStatic    whether this mesh is static or not.
     *                    Allows for internal optimizations.
     * @param maxVertices the maximum number of vertices this mesh can hold
     * @param maxIndices  the maximum number of indices this mesh can hold
     * @param attributes  the {@link VertexAttribute}s. Each vertex
     *                    attribute defines one property of a vertex such as position,
    ...
    public Mesh(boolean isStatic, int maxVertices, int maxIndices,
        ...
        addManagedMesh(GameEngine.app, this);
    }
     ...
     * @param maxVertices the maximum number of vertices this mesh can hold
     * @param maxIndices  the maximum number of indices this mesh can hold
     * @param attributes  the {@link VertexAttributes}. Each vertex
     *                    attribute defines one property of a vertex such as position,
     *                    normal or texture coordinate
     */
    public Mesh(boolean isStatic, int maxVertices, int maxIndices,
    ...
    /**
     * by jw: Creates a new Mesh with the given attributes. Adds extra
     * optimizations for dynamic (frequently modified) meshes.
     *
     * @param staticVertices whether vertices of this mesh are static or not.
     *                       Allows for internal optimizations.
     * @param staticIndices  whether indices of this mesh are static or not.
     *                       Allows for internal optimizations.
     * @param maxVertices    the maximum number of vertices this mesh can hold
     * @param maxIndices     the maximum number of indices this mesh can hold
    ...
    /**
     * Create a new Mesh that is a combination of transformations of the
     * supplied base mesh. Not all primitive types, like line
     * strip and triangle strip, can be combined.
     *
     * @param isStatic        whether this mesh is static or not. Allows
     ...
     * meshes must have the same VertexAttributes signature.
    ...
    /**
     * Create a new Mesh that is a combination of the supplied meshes. The
     * meshes must have the same VertexAttributes signature. If
     *
    ...
    /**
     * Sets the vertices of this Mesh. The attributes are assumed to be
     * given in float format.
     *
    ...
    /**
     * Sets the vertices of this Mesh. The attributes are assumed to
     * be given in float format.
     *
     * @param vertices the vertices.
     ...
     * The float array must be large enough to hold the
     * remaining vertices.
     *
     ...
     * The float array must be large enough to hold count vertices.
     *
     * @param srcOffset the offset (in number of floats) of the vertices
     *                  in the mesh to copy
    ...
    /**
     * Copies the specified vertices from the Mesh to the float array.
     * The float array must be large enough to hold
     ...
     * @param count      the amount of floats to copy
     * @param vertices   the array to copy the vertices to
     * @param destOffset the offset (in floats) in the vertices array
     *                   to start copying
     */
    public float[] getVertices(int srcOffset, int count, float[] vertices,
                               int destOffset) {
        ...
        if ((vertices.length - destOffset) < count)
            throw new IllegalArgumentException("not enough room in vertices array, has "
                    + vertices.length + " floats, needs "
                    + count);
     ...
     * be large enough to hold all the Mesh's indices.
     *
     ...
     * must be large enough to hold destOffset + all the
     * Mesh's indices.
     *
     * @param indices    the array to copy the indices to
    ...
    public void getIndices(short[] indices, int destOffset) {
        if ((indices.length - destOffset) < getNumIndices())
            throw new IllegalArgumentException("not enough room in indices " +
                    "array, has " + indices.length + " floats, needs "
                    + getNumIndices());
     ...
     * render methods. Usually you want to use autobind. Manual binding
     * is an expert functionality. There is a driver bug on the
     * MSM720xa chips that will fuck up memory if you manipulate the
     * vertices and indices of a Mesh multiple times while it is
     ...
     * {@link IndexBufferObject} if indices where given. Use this with OpenGL
     * ES 2.0 and when auto-bind is disabled.
     *
     * @param shader    the shader (does not bind the shader)
     ...
     * {@link IndexBufferObject} is indices were given. Use this with OpenGL
     ...
     * are set for this mesh then getNumIndices() / #vertices per
     * primitive primitives are rendered. If no indices are set then
     * getNumVertices() / #vertices per primitive are rendered.
     * </p>
     * <p>
     * <p>
     * This method will automatically bind each vertex attribute as
     * specified at construction time via {@link VertexAttributes} to
     * the respective shader attributes. The binding is based on the
    ...
    public void render(ShaderProgram shader, int primitiveType) {
        render(shader, primitiveType, 0,
                indices.getNumMaxIndices() > 0 ? getNumIndices() : getNumVertices(), autoBind);
    }

    /**
     * <p>
     * Renders the mesh using the given primitive type. offset specifies the
     * offset into either the vertex buffer or the index
     * buffer depending on whether indices are defined. count specifies the
     * number of vertices or indices to use thus count /
     * #vertices per primitive primitives are rendered.
     * </p>
     * <p>
     * <p>
     * This method will automatically bind each vertex attribute as specified
     * at construction time via {@link VertexAttributes} to
     * the respective shader attributes. The binding is based on the alias
     * defined for each VertexAttribute.
     * </p>
     * <p>
     * <p>
     * This method must only be called after the {@link ShaderProgram#begin()}
     ...
     * <p>
     * <p>
     * This method is intended for use with OpenGL ES 2.0 and will throw
     * an IllegalStateException when OpenGL ES 1.x is used.
     * </p>
     *
    ...
    public void render(ShaderProgram shader, int primitiveType, int offset,
        ...
        if (isVertexArray) {
            if (indices.getNumIndices() > 0) {
                ...
                GameEngine.gl20.glDrawElements(primitiveType, count,
                        IGL20.GL_UNSIGNED_SHORT, buffer);
                buffer.position(oldPosition);
                buffer.limit(oldLimit);
            } else {
                GameEngine.gl20.glDrawArrays(primitiveType, offset, count);
            }
        } else {
            if (indices.getNumIndices() > 0)
                GameEngine.gl20.glDrawElements(primitiveType, count,
                        IGL20.GL_UNSIGNED_SHORT, offset * 2);
            else
                GameEngine.gl20.glDrawArrays(primitiveType, offset, count);
        }
     ...
     */
    public void dispose() {
        if (meshes.get(GameEngine.app) != null)
            meshes.get(GameEngine.app).removeValue(this, true);
        vertices.dispose();
        indices.dispose();
    }
    ...
    /**
     * @return the backing FloatBuffer holding the vertices. Does not have
     * to be a direct buffer on Android!
     */
    ...
    /**
     * Calculates the {@link BoundingBox} of the vertices contained in
     * this mesh. In case no vertices are defined yet a
     * {@link com.guidebee.game.GameEngineRuntimeException}
    ...
    public void calculateBoundingBox(BoundingBox bbox) {
        ...
        if (numVertices == 0)
            throw new GameEngineRuntimeException("No vertices defined");
    ...
    public BoundingBox extendBoundingBox(final BoundingBox out, int offset,
        ...
        if (offset < 0 || count < 1 || offset + count > numIndices)
            throw new GameEngineRuntimeException("Not enough indices ( offset="
                    + offset + ", count=" + count + ", max=" + numIndices + " )");
     ...
     */
    public float calculateRadiusSquared(final float centerX, final float centerY,
                                        final float centerZ, int offset, int count,
                                        final Matrix4 transform) {
        int numIndices = getNumIndices();
        if (offset < 0 || count < 1 || offset + count > numIndices)
            throw new GameEngineRuntimeException("Not enough indices");
     ...
     */
    public float calculateRadius(final float centerX, final float centerY, final
    float centerZ, int offset, int count,
                                 final Matrix4 transform) {
        return (float) Math.sqrt(calculateRadiusSquared(centerX, centerY, centerZ,
    ...
    public float calculateRadius(final float centerX, final float centerY,
                                 final float centerZ, int offset, int count) {
        return calculateRadius(centerX, centerY, centerZ, offset,
                count, null);
    }
     ...
     */
    public float calculateRadius(final Vector3 center, int offset, int count) {
        return calculateRadius(center.x, center.y, center.z, offset,
                count, null);
    }
    ...
    public float calculateRadius(final float centerX, final float centerY,
                                 final float centerZ) {
        return calculateRadius(centerX, centerY, centerZ, 0,
                getNumIndices(), null);
    }
    ...
    /**
     * @return the backing shortbuffer holding the indices. Does not
     * have to be a direct buffer on Android!
     */
    ...
    /**
     * Invalidates all meshes so the next time they are rendered new
     * VBO handles are generated.
     *
     * @param app
     */
     ...
     * This is a potentially slow operation, use with
    ...
    // TODO: Protected for now, because transforming a portion works but
    // still copies all vertices
    ...
    /**
     * Method to transform the positions in the float array. Normals
     * will be kept as is. This is a potentially slow operation, use
     * with care.
     *
     * @param matrix     the transformation matrix
    ...
    public static void transform(final Matrix4 matrix, final float[] vertices,
                                 int vertexSize, int offset, int dimensions,
                                 int start, int count) {
        if (offset < 0 || dimensions < 1 || (offset + dimensions) > vertexSize)
            throw new IndexOutOfBoundsException();
        if (start < 0 || count < 1 || ((start + count) * vertexSize) > vertices.length)
            throw new IndexOutOfBoundsException("start = " + start + ", count = "
                    + count + ", vertexSize = " + vertexSize
     ...
     * potentially slow operation, use with care. It will also
    ...
    // TODO: Protected for now, because transforming a portion works
    // but still copies all vertices
     ...
     * @param count      the amount of vertices to transform
     */
    public static void transformUV(final Matrix3 matrix, final float[] vertices,
                                   int vertexSize, int offset, int start, int count) {
        if (start < 0 || count < 1 || ((start + count) * vertexSize) > vertices.length)
            throw new IndexOutOfBoundsException("start = " + start + ", count = "
                    + count + ", vertexSize = " + vertexSize
                    + ", length = " + vertices.length);
