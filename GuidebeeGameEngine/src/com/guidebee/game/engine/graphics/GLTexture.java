...
//[------------------------------ MAIN CLASS ----------------------------------]
/**
 * Class representing an OpenGL texture by its target and handle. Keeps track
 * of its state like the TextureFilter and TextureWrap.
 * Also provides some (protected) static methods to create TextureData and
 * upload image data.
 *
...
public abstract class GLTexture implements Disposable {
    ...
    /**
     * Binds this texture. The texture will be bound to the currently
     * active texture unit specified via
     * {@link com.guidebee.game.engine.graphics.opengles.IGL20#glActiveTexture(int)}.
     */
    public void bind() {
        GameEngine.gl.glBindTexture(glTarget, glHandle);
    }

    /**
     * Binds the texture to the given texture unit. Sets the currently
     * active texture unit via {@link com.guidebee.game.engine.graphics.opengles.IGL20#glActiveTexture(int)}.
     *
     * @param unit the unit (0 to MAX_TEXTURE_UNITS).
     */
    public void bind(int unit) {
        GameEngine.gl.glActiveTexture(IGL20.GL_TEXTURE0 + unit);
        GameEngine.gl.glBindTexture(glTarget, glHandle);
    }

    /**
     * @return The {@link com.guidebee.game.graphics.Texture.TextureFilter} used for minification.
     */
    ...
    /**
     * @return The {@link com.guidebee.game.graphics.Texture.TextureFilter} used for magnification.
     */
    ...
    /**
     * @return The {@link com.guidebee.game.graphics.Texture.TextureWrap} used for horizontal
    ...
    /**
     * @return The {@link com.guidebee.game.graphics.Texture.TextureWrap} used for vertical
     ...
     * @param u     the u wrap
     * @param v     the v wrap
     * @param force True to always set the values, even if they are the same
     *              as the current values.
     */
    public void unsafeSetWrap(TextureWrap u, TextureWrap v, boolean force) {
        if (u != null && (force || uWrap != u)) {
            GameEngine.gl.glTexParameterf(glTarget,
                    IGL20.GL_TEXTURE_WRAP_S, u.getGLEnum());
            uWrap = u;
        }
        if (v != null && (force || vWrap != v)) {
            GameEngine.gl.glTexParameterf(glTarget,
                    IGL20.GL_TEXTURE_WRAP_T, v.getGLEnum());
            vWrap = v;
        }
    }
    ...
    /**
     * Sets the {@link TextureFilter} for this texture for minification
     * and magnification. Assumes the texture is bound and active!
     *
     ...
     * @param minFilter the minification filter
     * @param magFilter the magnification filter
     * @param force     True to always set the values, even if they are
     *                  the same as the current values.
     */
    public void unsafeSetFilter(TextureFilter minFilter,
                                TextureFilter magFilter, boolean force) {
        if (minFilter != null && (force
                || this.minFilter != minFilter)) {
            GameEngine.gl.glTexParameterf(glTarget,
                    IGL20.GL_TEXTURE_MIN_FILTER, minFilter.getGLEnum());
            this.minFilter = minFilter;
        }
        if (magFilter != null && (force
                || this.magFilter != magFilter)) {
            GameEngine.gl.glTexParameterf(glTarget,
                    IGL20.GL_TEXTURE_MAG_FILTER, magFilter.getGLEnum());
            this.magFilter = magFilter;
        }
    }
    ...
    /**
     * Sets the {@link TextureFilter} for this texture for minification
     * and magnification. This will bind this texture!
     *
     * @param minFilter the minification filter
     * @param magFilter the magnification filter
     */
    ...
    protected static TextureData createTextureData(FileHandle file,
                                                   Format format,
                                                   boolean useMipMaps) {
    ...
    public static void uploadImageData(int target, TextureData data, int miplevel) {
        ...
        if (data.getFormat() != pixmap.getFormat()) {
            Pixmap tmp = new Pixmap(pixmap.getWidth(),
                    pixmap.getHeight(), data.getFormat());
        ...
        GameEngine.gl.glPixelStorei(IGL20.GL_UNPACK_ALIGNMENT, 1);
        if (data.useMipMaps()) {
            MipMapGenerator.generateMipMap(target, pixmap,
                    pixmap.getWidth(), pixmap.getHeight());
        } else {
            GameEngine.gl.glTexImage2D(target, miplevel,
                    pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
