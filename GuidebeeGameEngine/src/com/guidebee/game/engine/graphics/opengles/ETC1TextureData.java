...
public class ETC1TextureData implements TextureData {
    ...
    public void prepare() {
        ...
        if (file == null && data == null)
            throw new GameEngineRuntimeException("Can only load once from ETC1Data");
        if (file != null) {
    ...
    public void consumeCustomData(int target) {
        if (!isPrepared)
            throw new GameEngineRuntimeException(
                    "Call prepare() before calling consumeCompressedData()");

        if (!GameEngine.graphics.supportsExtension("GL_OES_compressed_ETC1_RGB8_texture")) {
            ...
            if (useMipMaps) MipMapGenerator.generateMipMap(target, pixmap,
                    pixmap.getWidth(), pixmap.getHeight());
            pixmap.dispose();
            useMipMaps = false;
        } else {
            GameEngine.gl.glCompressedTexImage2D(target, 0, ETC1.ETC1_RGB8_OES,
                    width, height, 0, data.compressedData.capacity()
                    - data.dataOffset, data.compressedData);
            if (useMipMaps()) GameEngine.gl20.glGenerateMipmap(IGL20.GL_TEXTURE_2D);
        }
        data.dispose();
        data = null;
        isPrepared = false;
