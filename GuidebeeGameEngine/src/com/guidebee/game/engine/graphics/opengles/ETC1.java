...
public class ETC1 {
    ...
    public final static class ETC1Data implements Disposable {
        ...
        /**
         * the offset in bytes to the actual compressed data. Might be 16
         * if this contains a PKM header, 0 otherwise *
         */
        ...
        public ETC1Data(FileHandle pkmFile) {
            ...
            try {
                in = new DataInputStream(new BufferedInputStream(
                        new GZIPInputStream(pkmFile.read())));
                ...
                throw new GameEngineRuntimeException("Couldn't load pkm file '"
                        + pkmFile + "'", e);
            } finally {
        ...
        public void write(FileHandle file) {
            ...
            try {
                ...
                throw new GameEngineRuntimeException("Couldn't write PKM file to '"
                        + file + "'", e);
            } finally {
        ...
        public String toString() {
            if (hasPKMHeader()) {
                return (ETC1.isValidPKM(compressedData, 0) ? "valid" : "invalid")
                        + " pkm [" + ETC1.getWidthPKM(compressedData, 0)
    ...
    private static int getPixelSize(Format format) {
        ...
        throw new GameEngineRuntimeException("Can only handle RGB565 or RGB888 images");
    }
    ...
    public static ETC1Data encodeImage(Pixmap pixmap) {
        ...
        ByteBuffer compressedData = encodeImage(pixmap.getPixels(), 0,
                pixmap.getWidth(), pixmap.getHeight(), pixelSize);
    ...
    /**
     * Encodes the image via the ETC1 compression scheme. Only {@link Format#RGB565}
     * and {@link Format#RGB888} are supported. Adds
    ...
    public static ETC1Data encodeImagePKM(Pixmap pixmap) {
        int pixelSize = getPixelSize(pixmap.getFormat());
        ByteBuffer compressedData = encodeImagePKM(pixmap.getPixels(), 0,
                pixmap.getWidth(), pixmap.getHeight(), pixelSize);
        BufferUtils.newUnsafeByteBuffer(compressedData);
     ...
     * or {@link Format#RGB888} {@link Pixmap}. Does
    ...
    public static Pixmap decodeImage(ETC1Data etc1Data, Format format) {
        ...
        decodeImage(etc1Data.compressedData, dataOffset, pixmap.getPixels(), 0,
	...
	*/
    ...
    /**
     * Writes a PKM header to the {@link ByteBuffer}. Does not modify the
     ...
     * @param height the height in pixels
     */
    public static native void formatHeader(ByteBuffer header, int offset,
	...
	*/
     ...
     * modify the position or limit of the
     * {@link ByteBuffer} instances.
     *
     * @param compressedData the compressed image data in a direct native
     *                       order {@link ByteBuffer}
     * @param offset         the offset in bytes to the image data from the
     *                       start of the buffer
     ...
     */
    private static native void decodeImage(ByteBuffer compressedData, int offset,
                                           ByteBuffer decodedData, int offsetDec,
	...
	*/
    ...
    /**
     * Encodes the image data given as RGB565 or RGB888. Does not modify the position
     * or limit of the {@link ByteBuffer}.
     *
     ...
     */
    private static native ByteBuffer encodeImage(ByteBuffer imageData, int offset,
                                                 int width, int height, int pixelSize); /*
		...
		etc1_encode_image((etc1_byte*)imageData + offset, width, height, pixelSize,
		 width * pixelSize, compressedData);
	...
	*/
     ...
     * @return a new direct native order ByteBuffer containing the compressed image data
     */
    private static native ByteBuffer encodeImagePKM(ByteBuffer imageData, int offset,
                                                    int width, int height, int pixelSize); /*
		int compressedSize = etc1_get_encoded_data_size(width, height);
		etc1_byte* compressed = (etc1_byte*)malloc(compressedSize + ETC_PKM_HEADER_SIZE);
		etc1_pkm_format_header(compressed, width, height);
		etc1_encode_image((etc1_byte*)imageData + offset, width, height, pixelSize,
		 width * pixelSize, compressed + ETC_PKM_HEADER_SIZE);
