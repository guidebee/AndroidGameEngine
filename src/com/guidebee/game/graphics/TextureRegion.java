 ...
 * origin in the upper left corner with the x-axis
...
public class TextureRegion {
    ...
    /**
     * @param width  The width of the texture region. May be negative
     *               to flip the sprite when drawn.
     * @param height The height of the texture region. May be negative
     *               to flip the sprite when drawn.
     */
    ...
    /**
     * Constructs a region with the same texture and coordinates of the
     * specified region.
     */
    ...
    /**
     * Constructs a region with the same texture as the specified region
     * and sets the coordinates relative to the specified region.
     *
     * @param width  The width of the texture region. May be negative to
     *               flip the sprite when drawn.
     * @param height The height of the texture region. May be negative to
     *               flip the sprite when drawn.
     */
    ...
    public void setRegion(float u, float v, float u2, float v2) {
        ...
        // For a 1x1 region, adjust UVs toward pixel center
        // to avoid filtering artifacts on AMD GPUs when drawing very stretched.
    ...
    /**
     * Sets the texture to that of the specified region and sets the
     * coordinates relative to the specified region.
     */
    public void setRegion(TextureRegion region, int x, int y,
                          int width, int height) {
        texture = region.texture;
        setRegion(region.getRegionX() + x,
                region.getRegionY() + y, width, height);
    }
    ...
    /**
     * Offsets the region relative to the current region. Generally the
     * region's size should be the entire size of the texture in
     * the direction(s) it is scrolled.
     *
     * @param xAmount The percentage to offset horizontally.
     * @param yAmount The percentage to offset vertically. This is done
     *                in texture space, so up is negative.
     */
     ...
     * from the top left corner going to the right and ending at
     * the bottom right corner. Only complete tiles will be returned so if
     * the region's width or height are not a multiple of the
     * tile width and height not all of the region will be used. This will
     * not work on texture regions returned form a TextureAtlas
     * that either have whitespace removed or where flipped before the
     * region is split.
     *
    ...
    public TextureRegion[][] split(int tileWidth, int tileHeight) {
        ...
        for (int row = 0; row < rows; row++, y += tileHeight) {
            ...
            for (int col = 0; col < cols; col++, x += tileWidth) {
                tiles[row][col] = new TextureRegion(texture, x, y,
     ...
     * ending at the bottom right corner. Only complete tiles will be
     * returned so if the texture's width or height are not a
     * multiple of the tile width and height not all of the texture
     * will be used.
     *
