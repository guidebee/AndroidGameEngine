...
public class TideMapLoader
    ...
    @Override
    public TiledMap load(AssetManager assetManager, String fileName,
                         FileHandle tideFile, Parameters parameter) {
        try {
            return loadMap(root, tideFile,
                    new AssetManagerImageResolver(assetManager));
        } catch (Exception e) {
            throw new GameEngineRuntimeException("Couldn't load tilemap '"
                    + fileName + "'", e);
        }
     ...
     */
    private TiledMap loadMap(XmlReader.Element root, FileHandle tmxFile,
     ...
     * @throws IOException
     */
    private Array<FileHandle> loadTileSheets(XmlReader.Element root, FileHandle tideFile)
            throws IOException {
    ...
    private void loadTileSheet(TiledMap map, XmlReader.Element element,
                               FileHandle tideFile, ImageResolver imageResolver) {
        if (element.getName().equals("TileSheet")) {
            ...
            for (int y = marginY; y <= stopHeight; y += tileSizeY + spacingY) {
                for (int x = marginX; x <= stopWidth; x += tileSizeX + spacingX) {
                    TiledMapTile tile = new StaticTiledMapTile(new TextureRegion(texture, x, y,
                            tileSizeX, tileSizeY));
                    tile.setId(gid);
    ...
    private void loadLayer(TiledMap map, XmlReader.Element element) {
        if (element.getName().equals("Layer")) {
            ...
            TiledMapTileLayer layer = new TiledMapTileLayer(layerSizeX, layerSizeY,
                    tileSizeX, tileSizeY);
            layer.setName(id);
            ...
            for (int row = 0, rowCount = rows.size; row < rowCount; row++) {
                ...
                for (int child = 0, childCount = currentRow.getChildCount();
                    ...
                    if (name.equals("TileSheet")) {
                        ...
                        Cell cell = new Cell();
                        cell.setTile(
                                currentTileSet.getTile(firstgid
                                        + currentChild.getIntAttribute("Index")));
                        ...
                        for (int frameChild = 0, frameChildCount = frames.getChildCount();
                             frameChild < frameChildCount; frameChild++) {
                            ...
                            if (frameName.equals("TileSheet")) {
                                ...
                                frameTiles.add(
                                        (StaticTiledMapTile) currentTileSet.getTile(firstgid
                                                + frame.getIntAttribute("Index")));
                            }
                        }
                        Cell cell = new Cell();
