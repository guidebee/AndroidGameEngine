...
public final class Tween extends BaseTween<Tween> {
    ...
    private static final Map<Class<?>, TweenAccessor<?>> registeredAccessors
            = new HashMap<Class<?>, TweenAccessor<?>>();
     ...
     * <i>com.guidebee.game.tween.equations</i> package, but you can of course
    ...
    public Tween build() {
        ...
        if (accessor == null)
            accessor=getAccessorByBaseClass();
    ...
    private TweenAccessor<Object> getAccessorByBaseClass(){
        Iterator it = registeredAccessors.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry pairs = (Map.Entry)it.next();
            Class clazz =(Class) pairs.getKey();
            TweenAccessor<Object> tweenAccessor=(TweenAccessor<Object>)pairs.getValue();
            if(checkIfInstanceOf(clazz.getCanonicalName())){
    ...
    private boolean checkIfInstanceOf(String className){
        Class clazz =target.getClass().getSuperclass();
        while(clazz.getCanonicalName()!=Object.class.getCanonicalName()){
            if(className==clazz.getCanonicalName()) return true;
