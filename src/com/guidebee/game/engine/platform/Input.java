 ...
 * @author jshapcot
 */
public class Input implements com.guidebee.game.Input, OnKeyListener, OnTouchListener {
    ...
    public void getPlaceholderTextInput(final TextInputListener listener,
                                        final String title, final String placeholder) {
        handle.post(new Runnable() {
            public void run() {
                AlertDialog.Builder alert = new AlertDialog.Builder(context);
                alert.setTitle(title);
    ...
    public synchronized boolean isKeyPressed(int key) {
        if (key == com.guidebee.game.Input.Keys.ANY_KEY) {
            return keyCount > 0;
        }
        if (key < 0 || key > 255) {
            return false;
...
// onscreenKeyboard.setVisible(visible);
        handle.post(new Runnable() {
            public void run() {
                ...
                if (visible) {
                    View view = ((Graphics) app.getGraphics()).getView();
                    view.setFocusable(true);
                    view.setFocusableInTouchMode(true);
                    manager.showSoftInput(((Graphics) app.getGraphics()).getView(), 0);
                } else {
                    manager.hideSoftInputFromWindow(
                            ((Graphics) app.getGraphics()).getView().getWindowToken(), 0);
                }
            }
        });
    }
     ...
     * >SensorManager#getRotationMatrix(float[], float[], float[], float[])</a>.
     * Does not manipulate the matrix if the platform
    ...
    void registerSensorListeners() {
        if (config.useAccelerometer) {
            ...
            if (manager.getSensorList(Sensor.TYPE_ACCELEROMETER).size() == 0) {
                ...
                accelerometerListener = new SensorListener(this.nativeOrientation,
        ...
        if (config.useCompass) {
            ...
            if (sensor != null) {
                ...
                if (compassAvailable) {
                    compassListener = new SensorListener(this.nativeOrientation,
                            this.accelerometerValues, this.magneticFieldValues);
                    compassAvailable = manager.registerListener(compassListener,
     ...
     * when we register more than one Sensor to a single
     * SensorEventListener, we add one of these for each Sensor.
     * Could use an anonymous class, but I don't see any harm in
    ...
    private class SensorListener implements SensorEventListener {
        ...
        SensorListener(Orientation nativeOrientation, float[] accelerometerValues,
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (event.sensor.getType() == Sensor.TYPE_MAGNETIC_FIELD) {
                System.arraycopy(event.values, 0, magneticFieldValues,
                        0, magneticFieldValues.length);
            }
        }
    }
}
