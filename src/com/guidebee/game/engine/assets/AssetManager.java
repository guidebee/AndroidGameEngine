...
public class AssetManager implements Disposable {
    final ObjectMap<Class, ObjectMap<String, RefCountedContainer>>
    ...
    public AssetManager(FileHandleResolver resolver) {
        ...
        setLoader(SVGImage.class, new SVGImageLoader(resolver));
    ...
    public synchronized <T> T get(String fileName) {
        ...
        if (type == null) throw new GameEngineRuntimeException("Asset not loaded: "
                + fileName);
        ObjectMap<String, RefCountedContainer> assetsByType = assets.get(type);
        if (assetsByType == null) throw new GameEngineRuntimeException("Asset not loaded: "
                + fileName);
        RefCountedContainer assetContainer = assetsByType.get(fileName);
        if (assetContainer == null) throw new GameEngineRuntimeException("Asset not loaded: "
                + fileName);
        T asset = assetContainer.getObject(type);
        if (asset == null) throw new GameEngineRuntimeException("Asset not loaded: "
                + fileName);
        return asset;
    }
    ...
    public synchronized <T> Array<T> getAll(Class<T> type, Array<T> out) {
        ...
        if (assetsByType != null) {
            for (ObjectMap.Entry<String,
                    RefCountedContainer> asset : assetsByType.entries()) {
    ...
    /**
     * Removes the asset and all its dependencies, if they
     * are not used by other assets.
     *
    ...
    public synchronized void unload(String fileName) {
        ...
        // check if it's currently processed (and the first element
        // in the stack, thus not a dependency)
        ...
        // if it is reference counted, decrement ref count and check if we
        // can really get rid of it.
    ...
    public synchronized <T> boolean containsAsset(T asset) {
        ObjectMap<String, RefCountedContainer> typedAssets
                = assets.get(asset.getClass());
     ...
     * loader exists for the specific filename, the default
     * loader for that type is returned.
     *
     * @param type     The type of the loader to get
     * @param fileName The filename of the asset to get a loader for,
     *                 or null to get the default loader
     * @return The loader capable of loading the type and filename,
    ...
    public synchronized <T> void load(String fileName, Class<T> type,
        ...
        if (loader == null)
            throw new GameEngineRuntimeException("No loader for type: "
        ...
        // check if an asset with the same name but a different type
        ...
        for (int i = 0; i < loadQueue.size; i++) {
            ...
            if (desc.fileName.equals(fileName) && !desc.type.equals(type))
                throw new GameEngineRuntimeException("Asset with name '"
                        + fileName
        ...
        for (int i = 0; i < tasks.size(); i++) {
            ...
            if (desc.fileName.equals(fileName) && !desc.type.equals(type))
                throw new GameEngineRuntimeException("Asset with name '" + fileName
     ...
     * milliseconds, yielding the CPU to the loading thread
     * between updates. This may block for less time if all loading tasks
     * are complete. This may block for more time if the portion
    ...
    synchronized void injectDependencies(String parentAssetFilename,
                                         Array<AssetDescriptor> dependendAssetDescs) {
    ...
    private synchronized void injectDependency(String parentAssetFilename,
                                               AssetDescriptor dependendAssetDesc) {
    ...
    /**
     * Removes a task from the loadQueue and adds it to the task stack. If
     * the asset is already loaded (which can happen if it was
     * a dependency of a previously loaded asset) its reference count will
     * be increased.
     */
    private void nextTask() {
        AssetDescriptor assetDesc = loadQueue.removeIndex(0);

        // if the asset not meant to be reloaded and is already loaded,
        ...
        if (isLoaded(assetDesc.fileName)) {
            ...
            if (assetDesc.params != null && assetDesc.params.loadedCallback != null) {
                assetDesc.params.loadedCallback.finishedLoading(this,
                        assetDesc.fileName, assetDesc.type);
            }
            loaded++;
        } else {
            // else add a new task for the asset.
    ...
    private void addTask(AssetDescriptor assetDesc) {
        ...
        if (loader == null)
            throw new GameEngineRuntimeException("No loader for type: "
                    + ClassReflection.getSimpleName(assetDesc.type));
        ...
        if (task.update()) {
            ...
            if (task.cancel) {
                ...
                if (task.assetDesc.params != null
                        && task.assetDesc.params.loadedCallback != null) {
                    task.assetDesc.params.loadedCallback.finishedLoading(this,
                            task.assetDesc.fileName, task.assetDesc.type);
                }

                long endTime = TimeUtils.nanoTime();
                log.debug("Loaded: " + (endTime - task.startTime) / 1000000f
                        + "ms " + task.assetDesc);
            }
    ...
    private void handleTaskError(Throwable t) {
        ...
        if (tasks.isEmpty()) throw new GameEngineRuntimeException(t);
        ...
        if (listener != null) {
            ...
            throw new GameEngineRuntimeException(t);
        }
    }
     ...
     */
    public synchronized <T, P extends AssetLoaderParameters<T>>
     ...
     *
     * @param type   the type of the asset
     * @param suffix the suffix the filename must have for this
     *               loader to be used or null to specify the default loader.
     * @param loader the loader
     */
    public synchronized <T, P extends AssetLoaderParameters<T>>
    void setLoader(Class<T> type, String suffix,
        ...
        if (loaders == null) this.loaders.put(type, loaders
    ...
    /**
     * Sets an {@link AssetErrorListener} to be invoked in case
     * loading an asset failed.
     *
    ...
    public synchronized int getReferenceCount(String fileName) {
        Class type = assetTypes.get(fileName);
        if (type == null) throw new GameEngineRuntimeException("Asset not loaded: " + fileName);
