...
public class Encoder {
    ...
    class LiteralEncoder {
        class Encoder2 {
            ...
            public void Init() {
                com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(m_Encoders);
            }

            public void Encode(com.guidebee.game.engine.utils.compression.rangecoder.Encoder rangeEncoder, byte symbol) throws IOException {
                int context = 1;
                for (int i = 7; i >= 0; i--) {
                    int bit = ((symbol >> i) & 1);
            ...
            public void EncodeMatched(com.guidebee.game.engine.utils.compression.rangecoder.Encoder rangeEncoder, byte matchByte,
            ...
            public int GetPrice(boolean matchMode, byte matchByte, byte symbol) {
                ...
                if (matchMode) {
                    for (; i >= 0; i--) {
                        int matchBit = (matchByte >> i) & 1;
                        int bit = (symbol >> i) & 1;
                        price += com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8)
                                + context], bit);
                        context = (context << 1) | bit;
                        if (matchBit != bit) {
                            i--;
                ...
                for (; i >= 0; i--) {
                    int bit = (symbol >> i) & 1;
                    price += com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice(m_Encoders[context], bit);
                    context = (context << 1) | bit;
                }
                return price;
            }
    ...
    class LenEncoder {
        ...
        public void Init(int numPosStates) {
            com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_choice);
        ...
        public void Encode(com.guidebee.game.engine.utils.compression.rangecoder.Encoder rangeEncoder, int symbol, int posState)
                throws IOException {
        ...
        public void SetPrices(int posState, int numSymbols, int[] prices, int st) {
            int a0 = com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_choice[0]);
            int a1 = com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_choice[0]);
            int b0 = a1 + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_choice[1]);
            int b1 = a1 + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_choice[1]);
            int i = 0;
    ...
    com.guidebee.game.engine.utils.compression.rangecoder.Encoder _rangeEncoder = new com.guidebee.game.engine.utils.compression.rangecoder.Encoder();
    ...
    void Init() {
        ...
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_isMatch);
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_isRep0Long);
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_isRep);
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_isRepG0);
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_isRepG1);
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_isRepG2);
        com.guidebee.game.engine.utils.compression.rangecoder.Encoder.InitBitModels(_posEncoders);
    ...
    int GetRepLen1Price(int state, int posState) {
        return com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state])
                + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax)
                + posState]);
    }
    ...
    int GetPureRepPrice(int repIndex, int state, int posState) {
        int price;
        if (repIndex == 0) {
            price = com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state]);
            price += com.guidebee.game.engine.utils.compression.rangecoder.Encoder
                    .GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
        } else {
            price = com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_isRepG0[state]);
            if (repIndex == 1)
                price += com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG1[state]);
            else {
                price += com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_isRepG1[state]);
                price += com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
            }
        }
        return price;
    }
    ...
    int GetOptimum(int position) throws IOException {
        ...
        _optimum[1].Price = com.guidebee.game.engine.utils.compression.rangecoder.Encoder
        ...
        int matchPrice = com.guidebee.game.engine.utils.compression.rangecoder.Encoder
                .GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
        int repMatchPrice = matchPrice + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_isRep[_state]);
        ...
        int normalMatchPrice = matchPrice + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isRep[_state]);
        ...
        while (true) {
            ...
            int curAnd1Price = curPrice
                    + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax)
                    + posState])
            ...
            matchPrice = curPrice
                    + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax)
                    + posState]);
            repMatchPrice = matchPrice + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_isRep[state]);
            ...
            if (!nextIsChar && matchByte != currentByte) {
                ...
                if (lenTest2 >= 2) {
                    ...
                    int nextRepMatchPrice = curAnd1Price
                            + com.guidebee.game.engine.utils.compression.rangecoder.Encoder
                            .GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext])
                            + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice1(_isRep[state2]);
                    {
            ...
            if (newLen >= startLen) {
                normalMatchPrice = matchPrice + com.guidebee.game.engine.utils.compression.rangecoder.Encoder.GetPrice0(_isRep[state]);
    ...
    void FillDistancesPrices() {
        ...
        for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++) {
            ...
            for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
                _posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << com.guidebee.game.engine.utils.compression.rangecoder.Encoder.kNumBitPriceShiftBits);
