 ...
 * implementation, but may be acceptable for small maps and has the benefits that
 * keys and values can be accessed by index, which
 * makes iteration fast. Like {@link Array}, if ordered is false,  this class
...
public class ArrayMap<K, V> implements Iterable<ObjectMap.Entry<K, V>> {
     ...
     *                 the order of other elements in the arrays, which avoids a
     *                 memory copy.
     * @param capacity Any elements added beyond this will cause the
     *                 backing arrays to be grown.
     */
    ...
    /**
     * Creates a new map with {@link #keys} and {@link #values} of
     * the specified type.
     *
     * @param ordered  If false, methods that remove elements may change
     *                 the order of other elements in the arrays, which avoids a
    ...
    /**
     * Creates an ordered map with {@link #keys} and {@link #values} of
     * the specified type and a capacity of 16.
     */
     ...
     * map will have the same type of backing arrays and
     * will be ordered if the specified map is ordered. The capacity is set to
     * the number of elements, so any subsequent elements
     ...
     * comparison of each key in reverse order until the
    ...
    /**
     * Returns the key for the specified value. Note this does a comparison
     * of each value in reverse order until the specified
     * value is found.
     *
     ...
     * number of entries. This is useful to release memory when
     ...
     * number of additional entries. Useful before adding
    ...
    protected void resize(int newSize) {
        K[] newKeys = (K[]) ArrayReflection.newInstance(keys.getClass().getComponentType(),
                newSize);
        System.arraycopy(keys, 0, newKeys, 0, Math.min(size, newKeys.length));
        this.keys = newKeys;

        V[] newValues = (V[]) ArrayReflection.newInstance(values.getClass().getComponentType(),
                newSize);
     ...
     * smaller than the specified size, no action
     * is taken.
     */
     ...
     * time this method is called. Use the {@link Entries} constructor for
     ...
     * Note that the same iterator instance is returned each time
     * this method is called. Use the {@link Entries} constructor for
     * nested or multithreaded iteration.
     */
    public Keys<K> keys() {
        if (keysIter1 == null) {
    ...
    static public class Entries<K, V> implements Iterable<ObjectMap.Entry<K, V>>,
        ...
        public boolean hasNext() {
            if (!valid)
                throw new GameEngineRuntimeException("#iterator() cannot be used nested.");
