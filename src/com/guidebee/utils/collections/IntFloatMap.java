 ...
 * implementation is a cuckoo hash map using 3 hashes, random
 * walking, and a small stash for problematic keys. Null keys are not allowed.
 * No allocation is done except when growing the table
 * size. <br>
 * <br>
 * This map performs very fast get, containsKey, and remove (typically O(1),
 * worst case O(log(n))). Put may be a bit slower,
 * depending on hash collisions. Load factors greater than 0.91 greatly
 * increase the chances the map will have to rehash to the
...
public class IntFloatMap implements Iterable<IntFloatMap.Entry> {
     ...
     * This map will hold 25 items before growing the
     * backing table.
     */
    public IntFloatMap() {
        this(32, 0.8f);
     ...
     * initialCapacity * 0.8 items before growing the backing
     * table.
     */
     ...
     * This map will hold initialCapacity * loadFactor items
    ...
    public IntFloatMap(int initialCapacity, float loadFactor) {
        ...
        if (loadFactor <= 0) throw new IllegalArgumentException("loadFactor must be > 0: "
                + loadFactor);
    ...
    private void push(int insertKey, float insertValue, int index1, int key1,
     ...
     * key is not in the map, defaultValue + increment is
     * put into the map.
     */
    public float getAndIncrement(int key, float defaultValue, float increment) {
        ...
        if (key != keyTable[index]) {
            ...
            if (key != keyTable[index]) {
                index = hash3(key);
                if (key != keyTable[index]) return getAndIncrementStash(key,
     ...
     * done. If the map contains more items than the specified capacity, the next
    ...
    /**
     * Returns true if the specified value is in the map. Note this traverses the entire map
     * and compares every value, which may be
    ...
    /**
     * Returns the key for the specified value, or null if it is not in the map. Note this
     * traverses the entire map and compares
     ...
     * additional items. Useful before adding many
     ...
     * time this method is called. Use the {@link Entries} constructor for nested
     * or multithreaded iteration.
     */
