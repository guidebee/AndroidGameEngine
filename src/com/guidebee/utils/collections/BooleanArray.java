...
//[------------------------------ MAIN CLASS ----------------------------------]
/**
 * A resizable, ordered or unordered boolean array. Avoids the boxing that occurs
 * with ArrayList<Boolean>. It is less memory
 * efficient than {@link BitSet}, except for very small sizes. It more CPU efficient
 * than {@link BitSet}, except for very large
 * sizes or if BitSet functionality such as and, or, xor, etc are needed.
 * If unordered, this class avoids a memory copy when
...
public class BooleanArray {
     ...
     * The new array will be ordered if the specific array is
     * ordered. The capacity is set to the number of elements, so any subsequent
     ...
     * The capacity is set to the number of elements,
     ...
     * The capacity is set to the number of elements, so any
    ...
    public void addAll(boolean[] array, int offset, int length) {
        ...
        if (sizeNeeded > items.length) items = resize(Math.max(8,
                (int) (sizeNeeded * 1.75f)));
