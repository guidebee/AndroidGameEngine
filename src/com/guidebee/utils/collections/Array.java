...
public class Array<T> implements Iterable<T> {
     ...
     * type is not Object, this field may only be accessed
     ...
     *                 order of other elements in the array, which avoids a
     *                 memory copy.
     ...
     *
     * @param ordered  If false, methods that remove elements may change
     *                 the order of other elements in the array, which avoids a
     *                 memory copy.
     * @param capacity Any elements added beyond this will cause
    ...
    /**
     * Creates an ordered array with {@link #items} of the specified
     * type and a capacity of 16.
     */
     ...
     * and will be ordered if the specified array is ordered. The capacity
     * is set to the number of elements, so any subsequent
     ...
     * array. The new array will have the same type of backing
     * array. The capacity is set to the number of elements, so any
     * subsequent elements added will cause the backing array to be
     * grown.
     */
     ...
     * Creates a new array containing the elements in the specified array.
     * The new array will have the same type of backing array.
     * The capacity is set to the number of elements, so any subsequent
     * elements added will cause the backing array to be grown.
     *
     * @param ordered If false, methods that remove elements may change
     *                the order of other elements in the array, which avoids a
    ...
    public void add(T value) {
        T[] items = this.items;
        if (size == items.length) items = resize(Math.max(8,
                (int) (size * 1.75f)));
        items[size++] = value;
    }
    ...
    public T get(int index) {
        if (index >= size) throw new IndexOutOfBoundsException("index can't be >= size: "
                + index + " >= " + size);
        return items[index];
    }
    ...
    public void swap(int first, int second) {
        if (first >= size) throw new IndexOutOfBoundsException("first can't be >= size: "
                + first + " >= " + size);
        if (second >= size) throw new IndexOutOfBoundsException("second can't be >= size: "
                + second + " >= " + size);
        T[] items = this.items;
    ...
    /**
     * Returns an index of first occurrence of value in array or -1 if
     * no such value exists
     *
     * @param identity If true, == comparison will be used. If false,
     *                 .equals() comparison will be used.
     * @return An index of first occurrence of value in array or -1
    ...
    /**
     * Returns an index of last occurrence of value in array or -1
     * if no such value exists. Search is started from the end of an
     * array.
     *
     * @param identity If true, == comparison will be used. If false,
     *                 .equals() comparison will be used.
    ...
    public void removeRange(int start, int end) {
        if (end >= size) throw new IndexOutOfBoundsException("end can't be >= size: "
                + end + " >= " + size);
        if (start > end) throw new IndexOutOfBoundsException("start can't be > end: "
                + start + " > " + end);
        T[] items = this.items;
     ...
     * This is useful to release memory when many items have
     ...
     * number of additional items. Useful before adding many
     ...
     * array must have a size greater than 0, or a {@link com.guidebee.game.GameEngineRuntimeException} will be thrown.
     *
     ...
     *                   value use 1, for max value use size of array, using
     *                   0 results in runtime exception.
     * @return the value of the Nth lowest ranked object.
     * @see com.guidebee.utils.Select
     */
    public T selectRanked(Comparator<T> comparator, int kthLowest) {
        if (kthLowest < 1) {
            throw new GameEngineRuntimeException("nth_lowest must be greater than 0, 1 = first, 2 = second...");
        }
     ...
     *                   n is based on ordinal numbers, not array indices. for min
     *                   value use 1, for max value use size of array,
     *                   using 0 results in runtime exception.
     ...
     * Note that the same iterator instance is returned each
     * time this method is called. Use the {@link ArrayIterator} constructor
     ...
     * Remove is supported, but not between hasNext() and next(). Note
     * that the same iterable instance is returned each time this method is
     * called. Use the {@link Predicate.PredicateIterable}
     ...
     * If the array is already smaller than the specified size, no action is
     * taken.
     */
    ...
    /**
     * Returns the items as an array. Note the array is typed,
     * so the {@link #Array(Class)} constructor must have been used.
    ...
    static public class ArrayIterator<T> implements Iterator<T>, Iterable<T> {
        ...
        public boolean hasNext() {
            if (!valid) throw new GameEngineRuntimeException("#iterator() cannot be used nested.");
        ...
        public void remove() {
            if (!allowRemove) throw new GameEngineRuntimeException("Remove not allowed.");
            index--;
